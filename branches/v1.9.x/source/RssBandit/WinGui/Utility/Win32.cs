...
namespace RssBandit
{
	...
	internal static class Win32
	{
		...
		#region IE integration

		internal static class InternetExplorerIntegration
		{
			/// <summary>
			...
			/// aggregator.			
			/// </summary>
			public static void CheckAndRegisterMenuExtensions()
			{
				try
				{
					//if we are the default aggregator then remove that menu option since it is redundant
					if (Registry.IsInternetExplorerExtensionRegistered(IEMenuExtension.DefaultFeedAggregator))
						Registry.UnRegisterInternetExplorerExtension(IEMenuExtension.DefaultFeedAggregator);

					if (IEVersion.Major < 9 || IsFeatureLocalMachineLockdown == false)
					{
						if (!Registry.IsInternetExplorerExtensionRegistered(IEMenuExtension.Bandit))
							Registry.RegisterInternetExplorerExtension(IEMenuExtension.Bandit);
					}
					 
					if (IEVersion.Major >= 9 && IsFeatureLocalMachineLockdown &&
						Registry.IsInternetExplorerExtensionRegistered(IEMenuExtension.Bandit))
					{
						Registry.UnRegisterInternetExplorerExtension(IEMenuExtension.Bandit);
					}
				}
				catch (Exception ex)
				{
					_log.Debug("CheckAndRegisterMenuExtensions(): Unable to modify InternetExplorerExtension", ex);
				}
			}

			private static bool IsFeatureLocalMachineLockdown
			{
				get
				{
					var root = "HKCU";
					...
					{
						_log.Error("Failed to read '" + root +@"\...Internet Explorer\Main\FeatureControl\FEATURE_LOCALMACHINE_LOCKDOWN'", ex);
					}
